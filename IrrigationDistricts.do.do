...
// tabstat totalf_totalrev_mills , statistics( mean count ) by(name)
//
...
// tabstat totalfunds_taxshare, statistics( mean count ) by(name)
// tabstat totalfunds_taxshare, statistics( mean count ) by(year)
//
...
// tabstat growth_totalfunds_tax, statistics( mean count ) by(name)
// tabstat growth_totalfunds_tax, statistics( mean count ) by(year)
// /*There are likely obvious outliers when you do some estimates. Be sure to 
// 	review the raw data (either in Stata or on the FIT platform)
// 	to understand why this government reported a large increase decrease in 
...
// tabstat gf_cash_exp_ratio, statistics( mean count ) by(name)
// tabstat gf_cash_exp_ratio, statistics( mean count ) by(year)
